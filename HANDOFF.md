...
上次重新生成：**2026-04-23**，基于 `feature/p3-polish` 分支 P7 AI 集成 + P7.0.1 review fix wave。当前 release：v0.11.0-p7（P6.1.1 为 v0.10.1-p6.1.1）。
...
1. **Repo interface façade**（issue #64，三 reviewer 共识）：`MemoRepository` / `SingleNoteRepository` 当前是 `open class + nullable ctor + by lazy requireNotNull` 模式，仅为 test fake 友好。延迟 NPE 爆点 + 封装破坏。P7.1 抽 `MemoRepositoryReadApi` / `SingleNoteReadApi` interface，real repo 和 fake 分别实现。
2. **AI 流式响应（SSE）**：当前非流式，每次要等完整 response。P7.1 接 SSE `stream=true`，逐 token 追加 assistant 气泡。
3. **AI 会话持久化**：当前 ephemeral（杀进程就没）。P7.1 加 `chat_sessions` Room 表，支持跨启动恢复 + 多会话切换。
4. **429 rate-limit 映射**：见 BDD 场景 52 — 当前 429 走 `UNKNOWN` 默认分支。P7.1 加 `ErrorCode.RATE_LIMITED` + 指数退避建议文案。
5. **"AI 改写" action**：P7 只支持 AI 回答（只读）。P7.1 加 "把 AI 回答应用到当前笔记" diff 预览 + 保存按钮。
6. **i18n**：P7 字符串全硬编码中文（与项目既有风格一致）。P8 统一提取 strings.xml 时一并做。
...
| 字段 | 值 |
|---|---|
| 最近发布的 release | `v0.11.0-p7`（tag，2026-04-23） |
| Release 来源分支 | `feature/p3-polish`（tag 从这里打，**不是** `master`） |
| 当前分支 | `feature/p3-polish` |
| HEAD SHA | 见 `git rev-parse HEAD`（P7 + P7.0.1 fix wave 收口提交） |
| `master` 分支 | **陈旧**。停在 `98724d1`（P1 之前）。不要从它合并、不要从它 pull、不要从它打 release。 |
| `versionCode` / `versionName` | `12` / `0.11.0-p7`（`app/build.gradle.kts:17-18`） |
| 工具链 | Kotlin 2.0.10 · AGP 8.7.3 · JDK 17 · compileSdk/targetSdk 35 · minSdk 26 |
| Room schema | `v8` — P7 未新增表（ephemeral 对话不持久化；持久化见 P7.1 TODO #3）。schema 导出在 `app/schemas/dev.aria.memo.data.local.AppDatabase/{1..8}.json`。 |
| 单元测试 | **220 项** passed · 0 failed，23 个 test 文件（详见 §6，包含 P7 的 4 个 AI test 文件）。 |
| **CI/CD** | `.github/workflows/ci.yml`（P6.1.1 首次引入）— push/PR 自动跑 compile + unit test，gate 在 GitHub 侧。 |
| GitHub issues | **0 个 open** · 75 个 closed。 |
| 工作树状态 | 见 `git status --porcelain`。P7 release 后若 DIRTY 则说明 P7.1 已经开工。 |
...
| **P7** | `v0.11.0-p7`（当前） | **AI 问答集成**：data/ai（AiClient/AiSettingsStore/AiContextBuilder/AiDto，OpenAI-compatible，key 存 EncryptedSharedPreferences、FLAG_SECURE 扩展）+ ui/ai（AiChatScreen 含三段上下文切换 + AiChatViewModel 多轮 transcript ephemeral） + NoteListScreen `Psychology` 图标入口 + SingleNoteRow 长按 "问 AI" DropdownMenu 带 noteUid 深链。**Review 闭环**：1 High + 10 Medium + 5 Low（issue #60–#75），修到零 open。**220 unit test** / 0 failed（23 test 文件）。BDD 扩场景 44-53。 | #60–#75 |
