...
- **Legacy day-file (`note_files`) 不支持软删除**（Data-1 R16 / #306）。
  整个 `YYYY-MM-DD.md` 文件的内容只能**覆写**，不能逐条 tombstone。
  原因：legacy 模型把"一天的所有 entry"存在一个文件里，没有稳定 uid
  可以打 tombstone 标。要删某条 entry 必须重写整个文件 body，相当于
  覆写。
  - 单笔记格式（`single_notes`）支持 `tombstoned` 列做软删除。
  - 用户体感上的"删一条 legacy entry" = "用 EditScreen 重新保存
    去掉那条" = 覆写。
  - 不应该后悔加 `tombstoned` 列 —— 加了也没意义（一行一文件，整行
    本身就是一个 file content）。
