...
上次重新生成：**2026-04-24**，基于 `feature/p3-polish` 分支 P8 Widget 重做 + 自动刷新。当前 release：v0.12.0-p8（P7 为 v0.11.0-p7；P6.1.1 为 v0.10.1-p6.1.1）。
...
8. **版本**：versionCode 13 / versionName `0.12.0-p8`。测试 ≈240 项，详见 §6。
...
| 字段 | 值 |
|---|---|
| 最近发布的 release | `v0.12.0-p8`（tag，2026-04-24） |
...
| `versionCode` / `versionName` | `13` / `0.12.0-p8`（`app/build.gradle.kts:17-18`） |
...
| **P8** | `v0.12.0-p8`（当前） | **Widget 重做 + 自动刷新**：MemoWidget 从 2×2 快速写快照改 3×3 默认（可 resize 4×4）的**最近 20 条可滚动列表**；Memo+Today widget 顶部都加 🔄 手动刷新按钮 + ➕ 新建按钮；新建 `data/widget/WidgetRefresher.kt`（object + SupervisorJob，fire-and-forget，`runCatching` 保护写路径）+ `widget/RefreshActionCallback.kt`（Glance action）；全链路 hook：MemoRepository / SingleNoteRepository 所有写方法、PullWorker / PushWorker 成功路径、SettingsStore PAT 变更、冷启动 MainActivity.onCreate 都触发 `WidgetRefresher.refreshAll(context)`，250ms debounce 防闪烁。`ServiceLocator.appContext` 字段新增。BDD 扩场景 54-80（≈27 新条，总计约 90）。`≈240 unit test` / 0 failed。 | TBD（等 Agent 6 review 产出） |
...
gh release list --limit 3                                  # 榜首：v0.12.0-p8
