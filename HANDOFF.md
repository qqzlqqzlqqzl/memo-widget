...
上次重新生成：**2026-04-23**，基于 `feature/p3-polish` 分支 P6.1 改版轮。当前 release：v0.10.0-p6.1。
...
3. **legacy `recentEntriesAcrossDays` LIMIT 下推**：`dao.observeAll().first()` 仍全表读；语义上多 entry/文件无法直接 SQL LIMIT 等价，需新增 `observeRecentFiles(fileLimit)` + 内存合并。Widget/UI 新路径已走单笔记表，legacy 是次优但无数据丢失风险 → 低优先级。
4. **EditViewModel.prime() 直访 ServiceLocator DAO**：违反 UI→Repository 分层。加 `MemoRepository.getContentForPath(path): String?` 薄方法后改。
5. **EditViewModel 3-arg legacy test 构造器迁移**：`DoubleTapSaveTest` 挪到新 4-deps 构造（对齐 `EditViewModelSingleNoteTest`），然后删已标 `@Deprecated` 的 adapter。
6. **LegacyDay 排序精细化 OR 一键迁移**：LegacyDay 时间塌陷 MIDNIGHT，single-note 09:00 排在 legacy 23:00 上方。P6.2 要么按 `group.entries.maxOf { time }` 重算，要么推用户走一键迁移。见 BDD 场景 40 + issue #49。
7. **首装未配 PAT 写笔记 data-loss**：`SingleNoteRepository.create` 在 `NOT_CONFIGURED` 直接返错不写 Room（与 legacy `appendToday` 同行为，不是 P6.1 regression）。加"本地草稿"缓冲或"必须先配 PAT"门禁。
8. **Markor fork 评估**：若要吸收 Markor Markdown 语法高亮等成熟度，评估最小化 fork 成本（Apache 2.0；XML View 栈不是 Compose，需桥接或逐步迁移）。
...
| 字段 | 值 |
|---|---|
| 最近发布的 release | `v0.10.0-p6.1`（tag，2026-04-23） |
| Release 来源分支 | `feature/p3-polish`（tag 从这里打，**不是** `master`） |
| 当前分支 | `feature/p3-polish` |
| HEAD SHA | 见 `git rev-parse HEAD`（P6.1 收口提交） |
| `master` 分支 | **陈旧**。停在 `98724d1`（P1 之前）。不要从它合并、不要从它 pull、不要从它打 release。 |
| `versionCode` / `versionName` | `10` / `0.10.0-p6.1`（`app/build.gradle.kts:17-18`） |
...
| 单元测试 | **共 ~179 项**，22+ 个 test 文件（详见 §6）。 |
| GitHub issues | **0 个 open** · 59 个 closed（#1–#59 全部在 P3/P4/P4.2/P6/P6.1 的修复浪潮里关掉）。 |
| 工作树状态 | 见 `git status --porcelain`。P6.1 release 后若 DIRTY 则说明 P6.2 已经开工，查 §10。 |
